<template>
  <div class="phone-card">
    <div :class="['vue-tel-input', styleClasses, { disabled: disabled }, `phone-input-${status}`]">
      <label :class="`phone-label label-${status}`">رقم الهاتف</label>
      <input dir="rtl" v-model="phone" ref="input" :type="inputOptions.type" :autocomplete="inputOptions.autocomplete"
        ...
        :tabindex="inputOptions.tabindex" :aria-describedby="inputOptions['aria-describedby']" @blur="onBlur"
        @focus="onFocus" @input="onInput" @keyup.enter="onEnter" @keyup.space="onSpace" />
      <slot name="icon-right" />
      <div class="dropdown-relative" v-click-outside="clickedOutside" aria-label="Country Code Selector"
        aria-haspopup="listbox" :aria-expanded="open" role="button"
        ...
        <ul v-if="open" ref="list" class="vti__dropdown-list dropdown-left" :style="{ width: dropdownOptions.width }"
          :class="dropdownOpenDirection" role="listbox">
          <input v-if="dropdownOptions.showSearchBox" class="vti__input vti__search_box"
            ...
            @click.stop />
          <li class="d-flex justify-content-end" v-for="(pb, index) in sortedCountries" role="option"
  ...
  props: {
    ...
    status: {
      ...
      validator: function (value) {
        return ['default', 'active', 'filled'].indexOf(value) !== -1;
      },
    },
  },
  data() {
